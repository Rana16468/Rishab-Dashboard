...
import termsAndConditionApi from "@/redux/Api/settings/trams&conditionApi";
...
export default function TermsAndConditionsPage() {
  ...
  useEffect(() => {
    if (!isHydrated) return; // Don't fetch data until hydrated

    const fetchTermsAndConditionsData = async () => {
      try {
        setIsLoading(true);
        const response = await termsAndConditionApi.getTermsAndCondition();

        if (response.success && response.data) {
          // Set content from API response - use correct field name
          setContent(
            response.data.TermsConditions || response.data.content || "",
          );
        } else {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = { TermsConditions: content };
      const response =
        await termsAndConditionApi.createTermsAndCondition(payload);
